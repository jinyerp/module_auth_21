...
<form wire:submit.prevent="update" class="space-y-6">
    <div class="bg-white shadow-sm rounded-lg">
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-xs text-yellow-800">
                            이 등급을 사용중인 <strong>{{ number_format($data->user_count) }}명</strong>의 회원이 있습니다.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">
                        등급의 레벨 (높을수록 상위 등급)
                        @if(isset($data->user_count) && $data->user_count > 0)
                        <span class="text-yellow-600">- 변경 시 주의</span>
                        @endif
                    </p>
                    @error('form.level')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        ...
        <div class="px-4 py-5 sm:p-6">
            <h3 class="text-sm font-medium text-gray-900 mb-4">메타 정보</h3>
            <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                @if(isset($data->created_at))
                <div>
                    <dt class="text-xs font-medium text-gray-500">생성일</dt>
                    <dd class="mt-1 text-xs text-gray-900">{{ $data->created_at->format('Y-m-d H:i:s') }}</dd>
                </div>
                @endif
                ...
                <div>
                    ...
                    <dd class="mt-1 text-xs text-gray-900">{{ $data->updated_at->format('Y-m-d H:i:s') }}</dd>
                </div>
                @endif
                
                @if(isset($data->user_count))
                <div>
                    <dt class="text-xs font-medium text-gray-500">사용중인 회원 수</dt>
                    <dd class="mt-1 text-xs text-gray-900">{{ number_format($data->user_count) }}명</dd>
                </div>
                @endif
            </dl>
        </div>
